...
function navClass(active: boolean): string {
  return [
    "rounded-[var(--a-radius-sm)] px-3 py-1.5 text-[length:var(--a-text-sm)] font-medium",
...
export function EmployeePortalShell({
  ...
  const onConges = pathname === EMPLOYEE_PORTAL_HOME_PATH;
  const onBulletins = pathname.startsWith(EMPLOYEE_PORTAL_BULLETINS_PATH);
    ...
    <div className="flex min-h-screen flex-col bg-a-surface-1 text-a-fg">
      ...
      <header className="flex h-14 shrink-0 items-center justify-between bg-a-surface-2/80 px-[var(--a-space-5)] backdrop-blur-[20px] backdrop-saturate-[180%]">
        ...
        <nav className="flex items-center gap-2">
          <Link href={EMPLOYEE_PORTAL_HOME_PATH} className={navClass(onConges)}>
            Congés
          </Link>
          <Link
            href={EMPLOYEE_PORTAL_BULLETINS_PATH}
            className={navClass(onBulletins)}
          >
            Bulletins
          </Link>
          <AButton
