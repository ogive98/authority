...
export const HELP_MODULES: HelpModule[] = [
      ...
      },
      {
        ...
        steps: {
          fr: [
            ...
            "Health : workers désactivés (config) = DEGRADED, pas Offline — un DLQ reste CRITICAL.",
            "Clic droit (ou icône clé) sur Thunder Health / un check → actions inline (scan, diagnostics, dépanner, Deep) — reste sur `/thunder` ; journal partagé avec `/repair`.",
            "Workers : c=0 surlignées · Repair ; enable = REDIS_URL / THUNDER_WORKERS_ENABLED (pas de toggle UI) ; Incidents = breakers ouverts + CLOSED.",
            "Event Bus / Outbox : lien Repair si lag élevé ou DLQ > 0.",
            "Throughput : historique events/s rempli à chaque poll (ring 24).",
            "Scheduler : dérivé workers/jobs du snapshot (pas de cron API dédié).",
            "Activity Feed / Integrations : chargement one-shot + refresh manuel.",
          ...
          it: [
            ...
            "Health: workers disabilitati (config) = DEGRADED, non Offline — una DLQ resta CRITICAL.",
            "Clic destro (o icona chiave) su Thunder Health / un check → azioni inline (scan, diagnostica, ripara, Deep) — resta su `/thunder` ; journal condiviso con `/repair`.",
            "Workers: c=0 evidenziate · Repair ; enable = REDIS_URL / THUNDER_WORKERS_ENABLED (niente toggle UI) ; Incidents = breaker aperti + CLOSED.",
            "Event Bus / Outbox: link Repair se lag alto o DLQ > 0.",
            "Throughput: storico events/s riempito a ogni poll (ring 24).",
            "Scheduler: derivato da workers/jobs dello snapshot (nessuna API cron dedicata).",
            "Activity Feed / Integrations: caricamento one-shot + refresh manuale.",
  ...
  },
  {
    ...
    features: [
      ...
      {
        name: {
          fr: "Actions Repair depuis Thunder Health",
          it: "Azioni Repair da Thunder Health",
        },
        when: {
          fr: "Depuis `/thunder` — clic droit sur Health ou un check.",
          it: "Da `/thunder` — clic destro su Health o un check.",
        },
        steps: {
          fr: [
            "Clic droit → Scanner / Diagnostics / Dépanner critiques / Réparer sérieusement — exécution sur `/thunder`.",
            "Le journal session est partagé (Thunder + Repair) ; exécution live step-up reste dans `/repair`.",
            "Deep L2 : scan + plan + dry-run SAFE/LOW — pas d’auto-exécution live.",
          ],
          it: [
            "Clic destro → Scansiona / Diagnostica / Ripara critici / Ripara seriamente — esecuzione su `/thunder`.",
            "Il journal di sessione è condiviso (Thunder + Repair); esecuzione live step-up resta in `/repair`.",
            "Deep L2: scan + plan + dry-run SAFE/LOW — niente auto-esecuzione live.",
          ],
        },
      },
    ],
