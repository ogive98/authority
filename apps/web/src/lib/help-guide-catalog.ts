  ...
  },
  {
      ...
      },
      {
        ...
        steps: {
          fr: [
            ...
            "D283 : un décaissement AP avec RAS crée automatiquement une retenue CALCULATED (à valider ici).",
            "Après validation : Certificat → attestation texte locale + SHA-256 (D284) — pas un formulaire MF officiel.",
            "Préparer lot XML (période) : pack les retenues CERTIFICATE_READY → statut TEJ_PREPARED · download local · jamais de transmission (D285).",
          ],
          it: [
            ...
            "D283: un pagamento AP con RAS crea automaticamente una ritenuta CALCULATED (da validare qui).",
            "Dopo validazione: Certificato → attestazione testo locale + SHA-256 (D284) — non un modulo MF ufficiale.",
            "Preparare lotto XML (periodo): packa le ritenute CERTIFICATE_READY → stato TEJ_PREPARED · download locale · mai trasmissione (D285).",
          ],
        },
      },
      {
        name: {
          fr: "TEJ brouillon local (XML + hash)",
          it: "TEJ bozza locale (XML + hash)",
        },
        when: {
          fr: "Préparer un fichier TEJ local après validation Prefs tax.tej.",
          it: "Preparare un file TEJ locale dopo validazione Prefs tax.tej.",
        },
        steps: {
          fr: [
            "Validez `tax.tej` dans Préférences › Expertise (jamais inventer).",
            "Sur `/tax` → section « TEJ — brouillon local ».",
            "Saisissez une période (ex. 2026-Q3) → Générer brouillon.",
            "Le XML se télécharge localement ; historique SHA-256 conservé.",
            "Transmission toujours DISABLED — pas d’upload ni d’API fiscale.",
            "Schéma = brouillon AUTHORITY (non officiel) jusqu’à XSD validé.",
          ],
          it: [
            "Validare `tax.tej` in Preferenze › Expertise (mai inventare).",
            "Su `/tax` → sezione « TEJ — brouillon local ».",
            "Inserire un periodo (es. 2026-Q3) → Générer brouillon.",
            "L’XML si scarica in locale; storico SHA-256 conservato.",
            "Trasmissione sempre DISABLED — nessun upload né API fiscale.",
            "Schema = bozza AUTHORITY (non ufficiale) fino a XSD validato.",
          ],
        },
      },
    ],
    locks: {
      fr: [
        ...
        "D283 : décaissement AP RAS → TaxWithholding CALCULATED idempotent (1:1 apPaymentId).",
        "D284 : certificat RAS local CERTIFICATE_READY (texte + SHA-256) — pas formulaire MF · pas TEJ.",
        "D285 : lot XML local WITHHOLDING_PACK depuis CERTIFICATE_READY → TEJ_PREPARED — pas XSD officiel · pas transmission.",
      ...
      it: [
        ...
        "D283: pagamento AP RAS → TaxWithholding CALCULATED idempotente (1:1 apPaymentId).",
        "D284: certificato RAS locale CERTIFICATE_READY (testo + SHA-256) — non modulo MF · no TEJ.",
        "D285: lotto XML locale WITHHOLDING_PACK da CERTIFICATE_READY → TEJ_PREPARED — no XSD ufficiale · no transmission.",
