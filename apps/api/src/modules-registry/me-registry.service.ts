...
export class MeRegistryService {
  ...
  async buildForUser(
    ...
    for (const row of states) {
      ...
      const candidates = buildFeatures(
        row.moduleKey,
      ...
      for (const f of candidates) {
        if (f.permissionKey) {
          const ok = await this.permissions.evaluate(userId, f.permissionKey, {
            companyId,
          });
          if (!ok) continue;
        }
...
function parseNavEntry(raw: Record<string, unknown>): RegistryFeature | null {
  ...
  const permissionKey =
    typeof raw.permissionKey === 'string' ? raw.permissionKey : undefined;
  return { id, label, href, flagKey, permissionKey };
}
