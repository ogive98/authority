...
import { CUSTOMERS_ERROR_CODES, CUSTOMERS_EVENT_TYPES } from './customers.constants';
import { CustomerFiscalService } from './customer-fiscal.service';
import { CustomersException } from './customers.exception';

describe('CustomerFiscalService (D260)', () => {
  ...
  function build(opts?: {
      ...
      opts?.override === undefined
        ...
        : opts.override === null
          ...
          : {
              ...
              customerId,
              taxCodeId,
              mode: CusFiscalOverrideMode.NEVER,
    ...
    return { service, prisma, outbox, customers };
  }

  it('upserts a fiscal profile and emits customers.fiscal.updated.v1', async () => {
    const { service, outbox } = build({ profile: null, override: null });
    const dto = await service.upsertProfile(companyId, customerId, {
      fiscalRegime: 'réel',
      vatLiable: true,
      withholdingArEnabled: false,
      version: 0,
    });
    expect(dto.taxId).toBe('1234567A');
  ...
  it('rejects NEVER without justification', async () => {
    ...
    await expect(
      service.upsertOverride(companyId, customerId, {
        taxCodeId,
        mode: 'NEVER',
        justification: '   ',
      }),
      ...
      response: { code: CUSTOMERS_ERROR_CODES.FISCAL_JUSTIFICATION },
    });
    expect.assertions(1);
  });
  ...
  it('rejects NEVER with SYSTEM_RULE source', async () => {
    const { service } = build({ override: null });
    try {
      await service.upsertOverride(companyId, customerId, {
        taxCodeId,
        mode: 'NEVER',
        source: 'SYSTEM_RULE',
        justification: 'test',
      });
      throw new Error('expected CustomersException');
    } catch (error) {
      expect(error).toBeInstanceOf(CustomersException);
      expect((error as CustomersException).code).toBe(
