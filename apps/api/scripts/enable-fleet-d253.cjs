...
const PERMS = ['fleet.manage', 'fleet.assign'];
...
(async () => {
  ...
  for (const c of companies) {
    await p.modModuleState.upsert({
      where: {
        companyId_moduleKey: { companyId: c.id, moduleKey: 'fleet' },
      },
    ...
    console.log('ENABLED fleet for', c.code);
  ...
  // Demo ADV users that already have delivery.read
  const deliveryGrants = await p.iamGrant.findMany({
    where: {
      permissionKey: 'delivery.read',
      subjectType: IamGrantSubject.USER,
      status: IamLifecycleStatus.ACTIVE,
    },
    select: { subjectId: true, companyId: true },
  });

  for (const g of deliveryGrants) {
    for (const permissionKey of PERMS) {
      await upsertGrant({
        permissionKey,
        subjectType: IamGrantSubject.USER,
  ...
  console.log(
    'USER fleet grants mirrored from delivery.read:',
    deliveryGrants.length,
  );

  const states = await p.modModuleState.findMany({
    where: { moduleKey: 'fleet' },
  });
  console.log('fleet states now:', states.map((s) => s.status).join(','));
